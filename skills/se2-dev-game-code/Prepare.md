...
- Subsequent runs detect game updates automatically: if the game's version changes, the previous `Decompiled/`, `Content/`, `CodeIndex/` and `graphify-out/` directories are wiped and rebuilt; the previous version stays available in the Git history.
