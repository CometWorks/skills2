...
Space Engineers 2 modding happens on both Windows and Linux, so keep scripts
portable. The Python scripts (`uv run ...`) behave identically everywhere; only
the ad-hoc shell commands differ.

### On Linux

Use the native shell directly with normal POSIX paths:
...
### On Windows: Using BusyBox

BusyBox provides the same UNIX-like commands on Windows:
...
#### Critical: Path Handling on Windows
...
#### Using PowerShell Instead (Windows)
...
5. **Keep scripts cross-platform** - Prefer Python/`pathlib` over shelling out; verify scripts work on both Windows and Linux
...
### Shelling out to grep from Python (cross-platform)
...
# On Windows, grep is provided by busybox.exe; on Linux it is native.
GREP = ["busybox.exe", "grep"] if sys.platform == "win32" else ["grep"]

def grep_decompiled(pattern):
    """Search decompiled source using grep"""
    # Forward slashes work on every platform (required on Windows/busybox).
    decompiled = Path("Data/Decompiled").resolve().as_posix()

    result = subprocess.run(
        [*GREP, "-r", "-n", pattern, decompiled],
        capture_output=True,
        text=True
    )
